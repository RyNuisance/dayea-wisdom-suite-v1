...
class NVDClient:
    ...
    def search_by_keyword(self, keyword: str, max_results: int = 10) -> list:
        ...
        try:
            ...
            self.logger.warning(f"     ⚠️  Cannot reach NVD API — no internet connection")
            return []
        except requests.exceptions.Timeout:
            self.logger.warning(f"     ⚠️  NVD API timed out — will retry later")
            return []
    ...
    def _query_nvd(self, keyword: str, max_results: int) -> list:
        ...
        response = requests.get(
            ...
            headers={"User-Agent": "DayeaWisdomSuite/1.0 Security Research"}
        )
